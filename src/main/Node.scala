...
class Node(pApp: PApplet, controlP5: ControlP5) extends Circle(pApp) {
  diameter = 60.0f
  ...
  protected var _plusBubble = new PlusBubble(pApp, controlP5, this)
  protected var _descriptionBubble = new DescriptionBubble(pApp, controlP5, this)
  ...
  def mousePressed(user: User): Unit = {
    ...
    _descriptionBubble.mousePressed(user)
    centeredOn(user)
  }

  def keyPressed: Unit = {
    _descriptionBubble.keyPressed
  }
  ...
  def centeredOn(user: User): Unit = {
    if(contains(new Vector(pApp.mouseX, pApp.mouseY))) {
      ...
      if(user.current._id != _id) {
        ...
        _descriptionBubble.visible = true
        _descriptionBubble.enabled = true

        user.current._descriptionBubble.visible = false
        user.current._descriptionBubble.enabled = false
...
*/
  ...
  def add(node: Node): Unit = {
    ...
    for(i <- 0 until _children.length) {
      ...
      _children(i)._descriptionBubble.visible = false
      _children(i)._descriptionBubble.enabled = false
    }
  }
  ...
  def move(delta: Vector): Unit = {
    ...
    _descriptionBubble.position.x += delta.x
    _descriptionBubble.position.y += delta.y
  ...
  override def position_=(value: Vector): Unit = {
    ...
    _plusBubble.setPositionByAngle(90)

    _descriptionBubble.setPositionByAngle(30)
  }
  ...
  def descriptionBubble = _descriptionBubble
  def depth = _depth
  def id = _id
  def parent = _parent
